...
import { ArrowLeft, Calendar, Clock, Share2, Bookmark, User, LogOut } from 'lucide-react'
...
export default function ArticlePage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--main-bg)] text-[var(--text-color)] pb-20 flex flex-col">
      {/* Hero Section - Flyer Style */}
      <div className="relative w-full min-h-[70vh] flex flex-col justify-center items-center p-6 pt-16 md:pt-20 pb-16 bg-grid-pattern">
            ...
            <div className="glass-card p-8 md:p-10 rounded-2xl border border-white/20 shadow-[0_0_40px_rgba(0,0,0,0.6)] text-center backdrop-blur-xl bg-black/50">
              ...
              <div className="flex flex-wrap justify-center items-center gap-4 mb-8 text-white/90">
                ...
                <span className="flex items-center gap-2 bg-white/10 px-4 py-1.5 rounded-full border border-white/10 text-sm font-medium backdrop-blur-md">
                  ...
                  {article.read_time || `${Math.max(1, Math.ceil(((article.content || article.description || '').trim().split(/\\s+/).length) / 200))} min read`}
                </span>
                <span className="flex items-center gap-2 bg-white/10 px-4 py-1.5 rounded-full border border-white/10 text-sm font-medium backdrop-blur-md">
